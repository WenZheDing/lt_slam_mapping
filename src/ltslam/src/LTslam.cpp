...
void LTslam::run(void)
{
    ...
    // gtsam::Pose3 test = isamCurrentEstimate.at<gtsam::Pose3>(genAnchorNodeIdx(source_sess_idx));
    ...
    gtsam::Pose3 test2 = isamCurrentEstimate.at<gtsam::Pose3>(genAnchorNodeIdx(source_sess_idx));
    cout << "test2:  " << test2.translation().x() << " " << test2.translation().y() << " " << test2.translation().z() << endl;

    // radius search loop detection (i.e., based on pose proximity) for non-SC-detected keyframes
...
#pragma omp parallel for num_threads(numberOfCores)
    for (int ith = 0; ith < num_scloops_sampled; ith++)
    {
        ...
        if (relative_pose_optional)
        {
            ...
            mtx.unlock();
            // cout << "relative_pose:  " << relative_pose.translation().x() << " " << relative_pose.translation().y() << " " << relative_pose.translation().z() << endl;
            // cout << "ith:  " << ith << endl;
...
void LTslam::findNearestRSLoopsTargetNodeIdx() // based-on information gain
{
    ...
    for (std::size_t i = 0; i < RSLoopIdxPairs_.size(); i++)
    {
        ...
        // cout << "query_sess_anchor_transform:  " << query_sess_anchor_transform.translation().x() << " " << query_sess_anchor_transform.translation().y() << " " << query_sess_anchor_transform.translation().z() << endl;
        // cout << "query_pose:  " << query_pose.translation().x() << " " << query_pose.translation().y() << " " << query_pose.translation().z() << endl;
        ...
        for (int target_node_idx = 0; target_node_idx < int(target_sess.nodes_.size()); target_node_idx++)
        {
            ...
            if (poseDistance(query_pose_central_coord, target_pose) < RSLoopsThreshold) // 10 is a hard-coding for fast test
            {
...
bool LTslam::addRSloops()
{
    ...
    {
        cout << "kNumRSLoopsUpperBound = 0! " << endl;
        return false;
    }
    ...
    {
        cout << "num_rsloops_all_found = 0! " << endl;
        return false;
    }
...
#pragma omp parallel for num_threads(numberOfCores)
    for (int ith = 0; ith < num_rsloops_sampled; ith++)
    {
        ...
        // 对于RS匹配上的 因为只是验证了距离 没有验证点云相似性 进行ICP 更新图
...
void LTslam::loadAllSessions()
{
    ...
    for (auto &_session_entry : fs::directory_iterator(sessions_dir_))
    {
        ...
        if (isTwoStringSame(session_name, central_sess_name_))
            session_idx = target_sess_idx; // 1
        else
            session_idx = source_sess_idx; // 2
